...
def test_tree_single_class_predicts_that_class():
    X = np.random.default_rng(0).normal(0, 1, (20, 3))
    y = np.zeros(20, dtype=int)
    t = DecisionTreeClassifier(random_state=0).fit(X, y)
    assert np.all(t.predict(X) == 0)


def test_tree_max_features_sqrt():
    X, y = _two_blobs()
    t = DecisionTreeClassifier(max_depth=5, max_features="sqrt", random_state=0).fit(X, y)
    assert t.predict(X).shape == y.shape


def test_tree_feature_mismatch_partial_fit_raises():
    t = DecisionTreeClassifier(random_state=0)
    t.partial_fit(np.ones((5, 3)), np.array([0, 1, 0, 1, 0]), classes=np.array([0, 1]))
    with pytest.raises(ValueError):
        t.partial_fit(np.ones((5, 4)), np.array([0, 1, 0, 1, 0]))
