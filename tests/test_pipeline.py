...
# --- Pipeline: transformer-only mode (A1 compatibility) ---
...
def test_pipeline_unique_names_required():
...
# --- Pipeline: estimator-terminated mode ---
...
def test_pipeline_estimator_requires_y():
    X, _ = _blobs()
...
def test_pipeline_transform_on_estimator_raises():
    X, y = _blobs()
    pipe = Pipeline([("scale", StandardScaler()),
                     ("tree", DecisionTreeClassifier(random_state=0))])
    pipe.fit(X, y)
...
def test_feature_union_partial_fit():
    X, _ = _blobs()
    fu = FeatureUnion([("s", StandardScaler()), ("m", MinMaxScaler())])
    for Xc, _ in iter_chunks(X, chunk_size=100):
        fu.partial_fit(Xc)
    out = fu.transform(X)
    assert out.shape == (X.shape[0], X.shape[1] * 2)
...
def test_stream_trainer_score_chunk_before_fit_raises():
    rf = RandomForestClassifier(n_estimators=4, random_state=0)
    tr = StreamTrainer(rf)
    with pytest.raises(RuntimeError):
        tr.score_chunk(np.ones((2, 4)), np.array([0, 1]))
